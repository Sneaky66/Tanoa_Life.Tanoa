...
switch (_code) do {
    ...
    case 19: {
		...
		//Handcuffing For Civs
		if(_shift && playerSide isEqualTo civilian && {!isNull cursorTarget} && {cursorTarget isKindOf "Man"} && {(isPlayer cursorTarget)} && {(side cursorTarget in [civilian,independent])} && {alive cursorTarget} && {cursorTarget distance player < 3.5} && {!(cursorTarget getVariable "Escorting")} && {!(cursorTarget getVariable "restrained")} && {speed cursorTarget < 1}) then {
